...
final class TypePeriod implements Type<Period> {
    ...
    @Override public void encode(final Encoder encoder, final Period value) throws IOException {
        encoder.escape(value.toString());
    }

    @Override public Period decode(final Decoder decoder, final Class<Period> clazz) throws IOException {
        return Period.parse(decoder.token(STRING));
    }
