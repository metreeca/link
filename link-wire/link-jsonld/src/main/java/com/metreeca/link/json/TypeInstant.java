...
final class TypeInstant implements Type<Instant> {
    ...
    @Override public void encode(final Encoder encoder, final Instant value) throws IOException {
        encoder.escape(value.toString());
    }

    @Override public Instant decode(final Decoder decoder, final Class<Instant> clazz) throws IOException {
        return Instant.parse(decoder.token(STRING));
    }
