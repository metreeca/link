...
final class TypeDuration implements Type<Duration> {
    ...
    @Override public Entry<Stream<Value>, Stream<Statement>> encode(final Encoder encoder, final Duration value) {
        return entry(Stream.of(encoder.factory().createLiteral(value)), Stream.empty());
    }

    @Override public Optional<Duration> decode(final Decoder decoder, final Value value, final Duration model) {
        return Optional.of(value)

                .filter(Value::isLiteral)
                .map(Literal.class::cast)

                .filter(v -> v.getDatatype().equals(XSD.DURATION))
